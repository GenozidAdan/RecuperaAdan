...
const AdminLayout = () => {
  ...
  const Admin = user?.roles[0]?.name === 'ADMIN_ROLE';
  const Client = user?.roles[0]?.name === 'CLIENT_ROLE';
  const User = user?.roles[0]?.name === 'USER_ROLE';

  if (!(Admin || Client || User)) {

    return <Navigate to="/error" />;
  }
  const nombreUsuario = `${user?.person?.name ?? ''}`;
